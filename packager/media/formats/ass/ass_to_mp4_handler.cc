...
#include <packager/media/formats/ass/ass_to_mp4_handler.h>
...
Status assToMp4Handler::InitializeInternal() {
  return Status::OK;
}

Status assToMp4Handler::Process(std::unique_ptr<StreamData> stream_data) {
    ...
    default:
      return Status(error::INTERNAL_ERROR,
                    "Invalid stream data type (" +
                        ...
                        ") for this assToMp4 handler");
  }
}

Status assToMp4Handler::OnStreamInfo(std::unique_ptr<StreamData> stream_data) {
  DCHECK(stream_data);
...
Status assToMp4Handler::OnCueEvent(std::unique_ptr<StreamData> stream_data) {
  DCHECK(stream_data);
  DCHECK(stream_data->cue_event);
  return Dispatch(std::move(stream_data));
}

Status assToMp4Handler::OnSegmentInfo(
...
Status assToMp4Handler::OnTextSample(std::unique_ptr<StreamData> stream_data) {
  DCHECK(stream_data);
