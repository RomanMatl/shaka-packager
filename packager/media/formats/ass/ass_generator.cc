...
std::string ToassTime(int64_t time, int32_t timescale) {
...
std::string ToassSize(const TextNumber& x, const TextNumber& y) {
...
const char* AssGenerator::kTtNamespace = "http://www.w3.org/ns/ass";

AssGenerator::AssGenerator() {}

AssGenerator::~AssGenerator() {}

void AssGenerator::Initialize(const std::map<std::string, TextRegion>& regions,
...
void AssGenerator::AddSample(const TextSample& sample) {
  samples_.emplace_back(sample);
}

void AssGenerator::Reset() {
  samples_.clear();
}
...
bool AssGenerator::AddSampleToXml(
  ...
  RCHECK(p.SetStringAttribute("begin",
                              ToassTime(sample.start_time(), time_scale_)));
  RCHECK(
      p.SetStringAttribute("end", ToassTime(sample.EndTime(), time_scale_)));
...
bool AssGenerator::ConvertFragmentToXml(
...
std::vector<std::string> AssGenerator::usedRegions() const {
...
bool AssGenerator::addRegions(xml::XmlNode& layout) const {
  ...
  for (const auto& r : regions_) {
    ...
    if (used) {
      xml::XmlNode region("region");
      const auto origin =
          ToassSize(r.second.window_anchor_x, r.second.window_anchor_y);
      const auto extent = ToassSize(r.second.width, r.second.height);
...
bool AssGenerator::isEbuTTTD() const {
  for (const auto& sample : samples_) {
